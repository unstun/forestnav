...
- 2026-07-05: 当前 formal gate 主任务书显式记录 F02.6 decision evidence matrix 边界。`module2_f02_6_decision_evidence_matrix` 当前状态为 `ready_for_dr_sun_decision_not_authorization`, 只支持 Dr Sun 在 `approve_obstacle_summary_warm_start` 与 `reject_obstacle_summary_warm_start` 两条路线之间做 F02.6 决策；matrix 已列出 invalid substitutes, 但它不是 local training、remote preflight、remote training、formal claim 或 paper-result material 授权。真实 formal gate 仍 blocked, 下一步仍只能记录 F02.6 决策, 不训练、不远端执行、不写结果性论文材料。
