...
- 2026-07-06 D5 complete: ran 6/6 local micro-smokes under `0_trials/module2_encoder_pilot/smoke_R*/` with 512 timesteps, 2 envs, and 2-episode evals; manifest/summary verification confirmed the R1-R6 extractor, sparse/dense reward, warm-start status, and non-unknown source heads; `PYTHONPATH=2_experiment KMP_DUPLICATE_LIB_OK=TRUE pytest 2_experiment/forest_n3p/tests/test_eval_rl_rs_gate3.py -q` passed with `2 passed in 1.79s`; full suite `PYTHONPATH=2_experiment KMP_DUPLICATE_LIB_OK=TRUE pytest 2_experiment/forest_n3p/tests -q` passed with `513 passed, 2 xfailed, 3 warnings in 64.44s`.
- 2026-07-06 D6 complete: ran remote R4 timing calibration on `gpu3070ti-relay` at `0_trials/module2_encoder_pilot/timing_R4_10k/train` with 10,000 requested timesteps, 8 envs, patch_cnn extractor, dense reward, and source head `a67794c5ac07570bf8d89658a8e68de49b60b1e7`; `/usr/bin/time -p` reported `real 420.78s`, i.e. about `23.77 steps/s`, projecting 500,000 timesteps to about `5.84h`; this is below the 12h downgrade threshold, so the matrix remains at 500,000 timesteps.
- 2026-07-06 D7 complete: ran R1-R6 sequentially on `gpu3070ti-relay` with 500,000 timesteps and 64-episode deterministic evals, source head `09259640169962f54af37bfa0cac9bd0b57d7270`; pulled artifacts locally under `0_trials/module2_encoder_pilot/R*/`; per-run verification confirmed remote execution host, non-unknown train/eval source heads, training manifests, checkpoint SHA records, final models, gate3 summaries, and 64 eval episode CSV rows.
- 2026-07-06 D8 complete: generated `0_trials/module2_encoder_pilot/pilot_summary.md` and `0_trials/module2_encoder_pilot/pilot_summary.csv`; the pre-registered decision rule selects R3 (`patch_cnn + no warm-start + sparse reward`) with `52/64` terminal-RS successes (`0.8125`) as the only run meeting the `>=0.80` freeze threshold; the summary explicitly marks these as pilot results, NOT formal, unlock no paper claims.
