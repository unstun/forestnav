...
# Module2 F02.6 Packet Remote Preflight Veto

## What Changed

The F02.6 warm-start decision packet now exposes a top-level
`remote_preflight_allowed=false` field.

This closes the remaining authorization ambiguity in the packet itself: the
artifact can list the post-approval remote route, but it now explicitly states
that even remote preflight is not currently authorized while F02.6 is still
pending.

The packet Markdown also prints `remote preflight allowed now: False`, and the
claim boundary now says the listed remote command is not current authorization
to preflight or train.
