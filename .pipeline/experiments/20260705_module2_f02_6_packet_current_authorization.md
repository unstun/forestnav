...
# Module2 F02.6 Packet Current Authorization

## What Changed

The F02.6 warm-start decision packet now consumes the F02.6 decision record and
decision intake audit as explicit sources and exposes a `current_authorization`
summary.

This separates two meanings that can otherwise be confused:

- The packet may describe post-approval routes.
- The packet is not current authorization to run remote preflight, train, claim
  results, or write paper-result material.

The current authorization summary states that the only allowed current action is
`record_f02_6_decision`; the blocked current actions are `remote_preflight`,
