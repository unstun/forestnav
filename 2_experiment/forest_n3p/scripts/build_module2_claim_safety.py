...
def _status_report_mainline_formal_gate_state_audit_summary(status_report: dict[str, Any]) -> dict[str, Any]:
    summary = status_report.get("mainline_formal_gate_state_audit_summary")
...
def _status_report_mainline_formal_gate_state_audit_blockers(status_report: dict[str, Any]) -> list[str]:
    summary = _status_report_mainline_formal_gate_state_audit_summary(status_report)
    blockers: list[str] = []
    if not summary["present"]:
        blockers.append("status_report_missing_mainline_formal_gate_state_audit_summary")
        return blockers
    if summary["status"] == "mainline_formal_gate_state_audit_failed":
        blockers.append("status_report_mainline_formal_gate_state_audit_failed")
    if not summary["not_paper_result_material"]:
        blockers.append("status_report_mainline_formal_gate_state_audit_marked_as_paper_result")
    if summary["executes_commands"]:
        blockers.append("status_report_mainline_formal_gate_state_audit_executes_commands")
    if summary["runs_training"]:
        blockers.append("status_report_mainline_formal_gate_state_audit_runs_training")
    if summary["runs_remote_preflight"]:
        blockers.append("status_report_mainline_formal_gate_state_audit_runs_remote_preflight")
    if summary["local_training_allowed"]:
        blockers.append("status_report_mainline_formal_gate_state_audit_allows_local_training")
    if summary["formal_claim_allowed"]:
        blockers.append("status_report_mainline_formal_gate_state_audit_allows_formal_claim")
    if summary["audit_issue_count"] > 0:
        blockers.append("status_report_mainline_formal_gate_state_audit_issues_open")
    if summary["proof_summary_chain_audit_issue_count"] > 0:
        blockers.append("status_report_mainline_proof_summary_issues_open")
    if summary["proof_summary_chain_proof_audit_input_safety_issue_count"] > 0:
        blockers.append("status_report_mainline_proof_audit_input_safety_issues_open")
    if "proof_audit_input_safety_issues_open" in summary["proof_summary_chain_proof_audit_blockers"]:
        blockers.append("status_report_mainline_proof_audit_input_safety_blocker_open")
    return blockers
...
def _status_report_decision_intake_summary(status_report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "decision_impact_summary_id": summary.get("decision_impact_summary_id"),
        ...
        "decision_impact_current_blocker": summary.get("decision_impact_current_blocker"),
...
def _status_report_decision_intake_blockers(status_report: dict[str, Any]) -> list[str]:
    ...
    if summary["decision_impact_present"] is not True:
        blockers.append("status_report_f02_6_decision_intake_impact_missing")
    if summary["decision_impact_not_paper_result_material"] is not True:
        blockers.append("status_report_f02_6_decision_intake_impact_is_paper_result_material")
    if summary["decision_record_is_not_training_authorization"] is not True:
        blockers.append("status_report_f02_6_decision_record_may_authorize_training")
    if summary["decision_record_is_not_paper_result_material"] is not True:
        blockers.append("status_report_f02_6_decision_record_may_be_paper_result_material")
    for field, blocker in (
        ("decision_impact_remote_preflight_allowed_now", "status_report_f02_6_decision_impact_allows_remote_preflight"),
        ("decision_impact_remote_training_allowed_now", "status_report_f02_6_decision_impact_allows_remote_training"),
        ("decision_impact_formal_claim_allowed_now", "status_report_f02_6_decision_impact_allows_formal_claim"),
        ("decision_impact_paper_result_material_allowed_now", "status_report_f02_6_decision_impact_allows_paper_result_material"),
    ):
...
def _handoff_single_next_action_index_summary(handoff_bundle: dict[str, Any]) -> dict[str, Any]:
    index = handoff_bundle.get("single_next_action_index")
    index = index if isinstance(index, dict) else {}
    return {
        "present": bool(index),
        "index_id": index.get("index_id"),
        "status": index.get("status"),
        "single_current_human_entry": index.get("single_current_human_entry")
        if isinstance(index.get("single_current_human_entry"), bool)
        else None,
        "next_action_id": index.get("next_action_id"),
        "decision_owner_required": index.get("decision_owner_required"),
        "valid_decisions": _strings(index.get("valid_decisions")),
        "required_record_fields": _strings(index.get("required_record_fields")),
        "current_allowed_action_ids": _strings(index.get("current_allowed_action_ids")),
        "current_blocked_action_ids": _strings(index.get("current_blocked_action_ids")),
        "post_decision_routes_are_current_authorization": index.get("post_decision_routes_are_current_authorization")
        if isinstance(index.get("post_decision_routes_are_current_authorization"), bool)
        else None,
        "all_execution_disabled_now": index.get("all_execution_disabled_now")
        if isinstance(index.get("all_execution_disabled_now"), bool)
        else None,
        "record_command_template_count": int(index.get("record_command_template_count") or 0),
        "local_training_allowed_now": index.get("local_training_allowed_now")
        if isinstance(index.get("local_training_allowed_now"), bool)
        else None,
        "remote_preflight_allowed_now": index.get("remote_preflight_allowed_now")
        if isinstance(index.get("remote_preflight_allowed_now"), bool)
        else None,
        "remote_training_allowed_now": index.get("remote_training_allowed_now")
        if isinstance(index.get("remote_training_allowed_now"), bool)
        else None,
        "formal_claim_allowed_now": index.get("formal_claim_allowed_now")
        if isinstance(index.get("formal_claim_allowed_now"), bool)
        else None,
        "paper_result_material_allowed_now": index.get("paper_result_material_allowed_now")
        if isinstance(index.get("paper_result_material_allowed_now"), bool)
        else None,
        "missing_deliverable_count": int(index.get("missing_deliverable_count") or 0),
        "open_category_count": int(index.get("open_category_count") or 0),
        "source_freshness_status": index.get("source_freshness_status"),
        "source_freshness_blocking_regeneration_required": index.get(
            "source_freshness_blocking_regeneration_required"
        )
        if isinstance(index.get("source_freshness_blocking_regeneration_required"), bool)
        else None,
        "approved_route_next_lane": index.get("approved_route_next_lane"),
        "rejected_route_next_lane": index.get("rejected_route_next_lane"),
        "after_approval_still_requires": _strings(index.get("after_approval_still_requires")),
    }


def _handoff_single_next_action_index_blockers(handoff_bundle: dict[str, Any]) -> list[str]:
    summary = _handoff_single_next_action_index_summary(handoff_bundle)
    blockers: list[str] = []
    if not summary["present"]:
        blockers.append("handoff_missing_single_next_action_index")
        return blockers
    if summary["index_id"] != "module2_formal_gate_single_next_action_index":
        blockers.append("handoff_single_next_action_index_id_invalid")
    pending_handoff = handoff_bundle.get("status") == "blocked_until_f02_6_decision"
    if pending_handoff and summary["status"] != "awaiting_dr_sun_f02_6_decision":
        blockers.append("handoff_single_next_action_index_status_not_pending_f02_6")
    if summary["status"] != "awaiting_dr_sun_f02_6_decision":
        return blockers
    expected_action = "record_f02_6_decision"
    if summary["single_current_human_entry"] is not True:
        blockers.append("handoff_single_next_action_index_not_single_human_entry")
    if summary["next_action_id"] != expected_action:
        blockers.append("handoff_single_next_action_index_next_action_not_decision")
    if summary["decision_owner_required"] != "Dr Sun":
        blockers.append("handoff_single_next_action_index_owner_not_dr_sun")
    if sorted(summary["valid_decisions"]) != [
        "approve_obstacle_summary_warm_start",
        "reject_obstacle_summary_warm_start",
    ]:
        blockers.append("handoff_single_next_action_index_valid_decisions_incomplete")
    if sorted(summary["required_record_fields"]) != ["decider", "decision", "decision_note"]:
        blockers.append("handoff_single_next_action_index_required_fields_incomplete")
    if summary["current_allowed_action_ids"] != [expected_action]:
        blockers.append("handoff_single_next_action_index_allowed_actions_not_decision_only")
    for blocked_action in (
        ...
        if blocked_action not in summary["current_blocked_action_ids"]:
            blockers.append(f"handoff_single_next_action_index_missing_block_{blocked_action}")
    if summary["post_decision_routes_are_current_authorization"] is not False:
        blockers.append("handoff_single_next_action_index_post_decision_routes_authorize_now")
    if summary["all_execution_disabled_now"] is not True:
        blockers.append("handoff_single_next_action_index_execution_not_disabled")
    if summary["record_command_template_count"] != 2:
        blockers.append("handoff_single_next_action_index_record_command_count_mismatch")
    if summary["local_training_allowed_now"] is not False:
        blockers.append("handoff_single_next_action_index_allows_local_training")
    if summary["remote_preflight_allowed_now"] is not False:
        blockers.append("handoff_single_next_action_index_allows_remote_preflight")
    if summary["remote_training_allowed_now"] is not False:
        blockers.append("handoff_single_next_action_index_allows_remote_training")
    if summary["formal_claim_allowed_now"] is not False:
        blockers.append("handoff_single_next_action_index_allows_formal_claim")
    if summary["paper_result_material_allowed_now"] is not False:
        blockers.append("handoff_single_next_action_index_allows_paper_result_material")
    if summary["missing_deliverable_count"] <= 0:
        blockers.append("handoff_single_next_action_index_zero_missing_deliverables_while_blocked")
    if summary["open_category_count"] <= 0:
        blockers.append("handoff_single_next_action_index_zero_open_categories_while_blocked")
    if summary["source_freshness_status"] != "source_freshness_clean_current":
        blockers.append("handoff_single_next_action_index_source_freshness_not_clean")
    if summary["source_freshness_blocking_regeneration_required"] is not False:
        blockers.append("handoff_single_next_action_index_source_freshness_blocks")
    if summary["approved_route_next_lane"] != "source_fresh_regeneration":
        blockers.append("handoff_single_next_action_index_approved_route_invalid")
    if summary["rejected_route_next_lane"] != "protocol_redesign":
        blockers.append("handoff_single_next_action_index_rejected_route_invalid")
    if "approved_remote_preflight" not in summary["after_approval_still_requires"]:
        blockers.append("handoff_single_next_action_index_approval_skips_remote_preflight")
    return blockers
...
def _status_report_remaining_deliverables_gap_summary(status_report: dict[str, Any]) -> dict[str, Any]:
    return _status_report_gap_summary(status_report.get("remaining_deliverables_gap_summary"))


def _status_report_formal_gate_gap_audit_remaining_deliverables_gap_summary(status_report: dict[str, Any]) -> dict[str, Any]:
    return _status_report_gap_summary(status_report.get("formal_gate_gap_audit_remaining_deliverables_gap_summary"))


def _status_report_gap_summary(raw_summary: Any) -> dict[str, Any]:
    summary = raw_summary
    summary = summary if isinstance(summary, dict) else {}
...
def _status_report_formal_gate_gap_audit_remaining_deliverables_gap_blockers(status_report: dict[str, Any]) -> list[str]:
    summary = _status_report_formal_gate_gap_audit_remaining_deliverables_gap_summary(status_report)
    ledger_summary = _status_report_remaining_deliverables_gap_summary(status_report)
    blockers: list[str] = []
    if not summary["present"]:
        blockers.append("status_report_missing_formal_gate_gap_audit_remaining_deliverables_gap_summary")
        return blockers
    if summary["summary_id"] != "module2_formal_gate_missing_training_eval_acceptance_summary":
        blockers.append("status_report_formal_gate_gap_audit_remaining_deliverables_gap_summary_id_invalid")
    if summary["category_order"] != list(STATUS_REPORT_REMAINING_DELIVERABLE_CATEGORY_IDS):
        blockers.append("status_report_formal_gate_gap_audit_remaining_deliverables_gap_category_order_mismatch")
    if ledger_summary["present"] and _gap_signature(summary) != _gap_signature(ledger_summary):
        blockers.append("status_report_formal_gate_gap_audit_remaining_deliverables_gap_mismatch")
    if summary["total_missing_deliverables"] > 0:
        blockers.append("status_report_formal_gate_gap_audit_remaining_deliverables_gap_rows_missing")
    if summary["open_category_count"] > 0:
        blockers.append("status_report_formal_gate_gap_audit_remaining_deliverables_gap_categories_blocked")
    if status_report.get("status") == "formal_gate_status_ready_for_claim_audit":
        if summary["total_missing_deliverables"] > 0:
            blockers.append("status_report_formal_gate_gap_audit_remaining_deliverables_gap_rows_missing_while_status_ready")
        if summary["open_category_count"] > 0:
            blockers.append("status_report_formal_gate_gap_audit_remaining_deliverables_gap_categories_blocked_while_status_ready")
    for category, payload in summary["categories"].items():
        if not payload["present"]:
            _append_unique(blockers, f"status_report_formal_gate_gap_audit_remaining_deliverables_gap_missing_{category}")
            continue
        if status_report.get("status") != "formal_gate_status_ready_for_claim_audit":
            if payload["responsible_stage_allowed_now"] is True:
                _append_unique(
                    blockers,
                    f"status_report_formal_gate_gap_audit_remaining_deliverables_gap_{category}_stage_allowed_while_blocked",
                )
        if payload["missing_count"] != len(payload["missing_artifact_matrix_ids"]):
            _append_unique(
                blockers,
                f"status_report_formal_gate_gap_audit_remaining_deliverables_gap_{category}_missing_artifact_count_mismatch",
            )
    return blockers


def _status_report_remote_packet_safety_claim_gate_command_index_summary(
    status_report: dict[str, Any],
) -> dict[str, Any]:
    summary = status_report.get("remote_packet_safety_claim_gate_command_index_summary")
    summary = summary if isinstance(summary, dict) else {}
    raw_rows = summary.get("claim_gate_rows") if isinstance(summary.get("claim_gate_rows"), dict) else {}
    rows: dict[str, dict[str, Any]] = {}
    for artifact_id in STATUS_REPORT_CLAIM_GATE_REGENERATION_ARTIFACT_IDS:
...
def _status_report_remote_packet_safety_claim_gate_command_index_blockers(
    status_report: dict[str, Any],
) -> list[str]:
    summary = _status_report_remote_packet_safety_claim_gate_command_index_summary(status_report)
    blockers: list[str] = []
    if not summary["present"]:
        blockers.append("status_report_missing_remote_packet_safety_claim_gate_command_index_summary")
        return blockers
    if summary["missing_target_ids"]:
        blockers.append("status_report_remote_packet_safety_command_index_missing_targets")
    if summary["unknown_manual_count"] > 0:
        blockers.append("status_report_remote_packet_safety_command_index_unknown_manual_rows")
    if summary["forbidden_command_count"] > 0:
        blockers.append("status_report_remote_packet_safety_command_index_forbidden_commands")
    for artifact_id, row in summary["claim_gate_rows"].items():
        if not row["present"]:
            _append_unique(blockers, f"status_report_remote_packet_safety_command_index_missing_{artifact_id}")
            continue
        if row["stage_id"] != "regenerate_claim_gate_artifacts":
            _append_unique(blockers, f"status_report_remote_packet_safety_command_index_{artifact_id}_wrong_stage")
        if row["required_before"] != "formal_claim_gate":
            _append_unique(
                blockers,
                f"status_report_remote_packet_safety_command_index_{artifact_id}_wrong_required_before",
            )
        if row["command_kind"] == "unknown_manual":
            _append_unique(blockers, f"status_report_remote_packet_safety_command_index_{artifact_id}_manual_command")
    return blockers
...
def _markdown(manifest: dict[str, Any]) -> str:
    ...
    lines.append(f"- matches_status_report_remaining_gap=`{_gap_signature(formal_gate_gap) == _gap_signature(gap)}`")
    remote_proof = manifest["status_report_remote_packet_safety_proof_deliverables_summary"]
    remote_status_proof = manifest["status_report_remote_packet_safety_status_report_proof_deliverables_summary"]
    lines.extend(["", "## Status Report Remote-Safety Proof Deliverables Summary", ""])
    ...
    lines.append(
        "- remote_proof_matches_gap_summary="
        f"`{_proof_deliverables_gap_signature(remote_proof) == _proof_deliverables_signature_from_gap(formal_gate_gap)}`"
    )
    for category, matrix_ids in remote_proof["missing_matrix_ids_by_formal_category"].items():
        missing_ids = ", ".join(matrix_ids) if matrix_ids else "none"
        lines.append(f"- remote_proof_{category}_missing_matrix_ids=`{missing_ids}`")
    command_index = manifest["status_report_remote_packet_safety_claim_gate_command_index_summary"]
    lines.extend(["", "## Status Report Remote-Safety Claim-Gate Command Index", ""])
