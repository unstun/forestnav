...
class VoronoiVerificationRecord:
    case_id: int
    profile_name: str
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify the T11 Voronoi waypoint Hybrid A* baseline.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(".pipeline/experiments/20260620_t11_voronoi_waypoint_verification"),
    )
    parser.add_argument(
        "--report-path",
        type=Path,
        default=Path(".pipeline/experiments/20260620_t11_voronoi_waypoint_verification.md"),
    )
    ...
    parser.add_argument("--waypoint-spacing-m", type=float, default=6.0)
...
def run_verification(args: argparse.Namespace) -> list[VoronoiVerificationRecord]:
    ...
    baseline_config = VoronoiWaypointConfig(
    ...
    records: list[VoronoiVerificationRecord] = []
    ...
    for case_id, profile in enumerate(schedule):
        ...
        query_seed = int(args.seed) + 710_000 + int(case_id)
        ...
        _plot_case(figure_path, grid_map, start, goal, result.path, result.waypoints)
        records.append(
            VoronoiVerificationRecord(
                case_id=int(case_id),
...
def render_report(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- 参数: waypoint spacing = {float(payload['config']['waypoint_spacing_m']):.1f} m, "
    ...
    for record in summary["by_case"]:
        lines.append(
            ...
            f"{record['skipped_waypoint_total']} | "
...
def _generate_case_map(
    ...
    for attempt in range(20):
        map_seed = int(args.seed) + 620_000 + int(case_id) * 1_000 + attempt
        map_rng = np.random.default_rng(map_seed)
        try:
            grid, _start_xy, _goal_xy = _run_with_wall_timeout(
                "verify_voronoi_map_generation",
...
def _plot_case(
    ...
    if waypoint_seq:
        ax.scatter([p[0] for p in waypoint_seq], [p[1] for p in waypoint_seq], s=46, marker="x", color="#ff7f0e", label="waypoints")
...
def _write_csv(path: Path, rows: Iterable[VoronoiVerificationRecord]) -> None:
    rows = tuple(rows)
