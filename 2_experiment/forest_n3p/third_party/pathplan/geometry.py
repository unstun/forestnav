...
class GridFootprintChecker:
    ...
    def __init__(
        ...
        self._pose_collision_cache: dict[Tuple[int, int, int], bool] = {}
        self._pose_collision_cache_max = 200_000
    ...
    def collides_pose(self, x: float, y: float, theta: float) -> bool:
        ...
        key = (int(gx), int(gy), int(theta_idx))
        cached = self._pose_collision_cache.get(key)
        if cached is not None:
            return bool(cached)
        collides = bool(self._collides_grid(gx, gy, theta_idx))
        if len(self._pose_collision_cache) >= self._pose_collision_cache_max:
            self._pose_collision_cache.clear()
        self._pose_collision_cache[key] = bool(collides)
