...
class RRTStarPlanner:
    ...
    def plan(
        ...
        # Use a monotonic clock for timeouts (Windows system time can jump).
    ...
    def _edge_poses_and_length(self, edge: SplineEdge, step: float) -> Tuple[List[AckermannState], float]:
        ...
        for seg_idx, seg in enumerate((edge.seg1, edge.seg2)):
            ...
            # Guard against degenerate spline constructions that yield extremely long control
            # polygons (which would otherwise produce millions of samples and blow past the
            # wall-clock timeout by a large margin).
