...
from forest_n3p.baselines.lian2023.corridor import build_corridor, is_corridor_degenerate
from forest_n3p.baselines.lian2023.eha_star import resample_path_to_n_points, run_eha_star
...
class LianPlannerConfig:
    ...
    # ----------------------------------------------------------------
    skip_nlp: bool = False
    use_corridor_goal_theta: bool = True
...
class LianPlanner:
    ...
    def plan(
        ...
        *,
        ...
        # Stage 1.3: 边界点选取
        # ----------------------------------------------------------------
        # caller 传入的 goal_state[2] 通常是占位 0.0（env 不知道实际接近角）。
        # 用 corridor 末段切向作为 goal 接近角，否则 BP_last 会与
        # 实际可达方向冲突，导致 EHA* 末段 HA* 在 tolerance 内永远找不到解。
        # 同理，start theta 若与 corridor 首段切向偏差过大也会失败，
        # 但 start theta 一般是 env 当前朝向（真实），保留 caller 值。
        # 诊断溯源：debug_lian2023_eha.py 2026-05-07
        # ================================================================
