...
接口
----
run_eha_star(...)         对相邻 BP 调用本目录内 classic Hybrid A*，拼接 segment
resample_path_to_n_points 将拼接路径线性插值重采样到固定 n_points 个点

path_states 来源
----------------
本目录内直接调用第三方 ``HybridAStarPlanner``。优先使用 planner 返回的
``trace_poses``；若缺失，则用离散 state 序列。
"""
...
# 内部依赖
# ===========================================================================
from forest_n3p.baselines.lian2023_paper_only.types import BoundaryPoint, PlanStatus
...
def run_eha_star(
    *,
    ...
    """对相邻 BP 调用本目录内 classic Hybrid A*，拼接所有 segment。
    ...
    goal_xy_tol_m:
        到达目标 XY 容忍半径（m），传给本目录内 classic Hybrid A*。
    goal_theta_tol_rad:
        到达目标朝向容忍（rad）。
...
def _plan_classic_segment(
    *,
    ...
    """Classic Hybrid A* segment used inside Lian 2023 EHA*.

    The planner is instantiated here so the paper-only package does not depend
    on any other baseline implementation.
    """
    ...
    planner = HybridAStarPlanner(
        ...
        footprint,
        params,
        primitives=default_primitives(params, step_length=0.3),
    ...
    if trace and len(trace) >= 2:
        return [(float(x), float(y), float(th)) for x, y, th in trace]
    return [(float(s.x), float(s.y), float(s.theta)) for s in path]
