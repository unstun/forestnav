...
# 内部依赖（各阶段模块）
# ===========================================================================
from forest_n3p.baselines.lian2023_paper_only.astar_2d import astar_2d_path
from forest_n3p.baselines.lian2023_paper_only.boundary_points import select_boundary_points
from forest_n3p.baselines.lian2023_paper_only.corridor import build_corridor, is_corridor_degenerate
from forest_n3p.baselines.lian2023_paper_only.eha_star import resample_path_to_n_points, run_eha_star
from forest_n3p.baselines.lian2023_paper_only.nlp import (
    NlpVarLayout,
    flatten_initial_guess,
    solve_nlp,
    unflatten_solution,
)
from forest_n3p.baselines.lian2023_paper_only.types import (
    CorridorSegment,
    PlanResult,
    PlanStatus,
    TrajPoint,
...
# ---------------------------------------------------------------------------
# 多圆车体几何提取（Lian 2023 §3.1 多圆覆盖）
# ---------------------------------------------------------------------------

def _disc_geometry_from_footprint(
    footprint: OrientedBoxFootprint | TwoCircleFootprint,
) -> tuple[tuple[float, ...], float]:
    """根据 footprint 推 (disc_offsets, disc_radius)。

    - TwoCircleFootprint：`(center_shift ± center_offset)` 沿车身轴向，
      圆半径 = `footprint.radius`。
    - OrientedBoxFootprint：参照 ``TwoCircleFootprint.from_box`` 构造，
      `center_offset = half_length / 2`，`radius = sqrt(center_offset² + half_width²)`，
      沿体心 ±center_offset。
    - 其他 / None：返回 ((), 0.0) → NLP 退化为单点模式。
    """
    if isinstance(footprint, TwoCircleFootprint):
        s = float(footprint.center_shift)
        o = float(footprint.center_offset)
        return (s + o, s - o), float(footprint.radius)
    if isinstance(footprint, OrientedBoxFootprint):
        half_l = float(footprint.half_length)
        half_w = float(footprint.half_width)
        center_offset = 0.5 * half_l
        radius = math.hypot(center_offset, half_w)
        return (+center_offset, -center_offset), float(radius)
    return (), 0.0
...
class LianPlanner:
    ...
    def plan(
        ...
        *,
        ...
        # Stage 1.3: 边界点选取
        # ----------------------------------------------------------------
        # paper-only 默认保留 caller 提供的目标航向。
        ...
        # footprint-implied 几何写入 stats 仅作 audit（不参与 NLP 约束）
        _disc_off_auto, _disc_r_auto = _disc_geometry_from_footprint(self.footprint)
        stats["disc_offsets_m_footprint_implied"] = list(_disc_off_auto)
        stats["disc_radius_m_footprint_implied"] = float(_disc_r_auto)
        ...
        nlp_res = solve_nlp(
            ...
            wheelbase_m=self.params.wheelbase,
