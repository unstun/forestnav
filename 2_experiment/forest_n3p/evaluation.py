    ...
    for method, bucket in keys:
        ...
        out.append(
            GroupSummary(
                ...
                mean_nn_forward_time_s=_mean(row.nn_forward_time_s for row in group),
                p95_nn_forward_time_s=_percentile((row.nn_forward_time_s for row in group), 95),
                rl_attempts_total=sum(int(row.rl_attempts or 0) for row in group),
                rl_successes_total=sum(int(row.rl_successes or 0) for row in group),
                rs_attempts_total=sum(int(row.rs_attempts or 0) for row in group),
                fallback_to_primitives_total=sum(int(row.fallback_to_primitives_count or 0) for row in group),
