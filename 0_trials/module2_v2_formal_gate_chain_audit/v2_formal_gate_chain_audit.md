...
- current_blocking_stage_id: `v2_remote_preflight_ready`
- next_allowed_action: `run_remote_preflight_only`
...
- `source_freshness_ready`: satisfied=`True`, observed=`source_freshness_remote_preflight_scope_ready_with_later_risks`, expected=`['source_freshness_clean_current', 'source_freshness_remote_preflight_scope_ready_with_later_risks', 'source_freshness_tracked_artifact_lag_only_gate_ready']`
- `v2_remote_packet_ready`: satisfied=`True`, observed=`{'status': 'ready_for_v2_remote_preflight', 'remote_preflight_allowed_now': True}`, expected=`{'status': 'ready_for_v2_remote_preflight', 'remote_preflight_allowed_now': True}`
